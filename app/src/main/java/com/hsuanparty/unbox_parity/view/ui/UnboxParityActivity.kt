...
class UnboxParityActivity : AppCompatActivity(), HasSupportFragmentInjector, Injectable {
    ...
    companion object {
        ...
        private const val HIDE_TOOLBAR = 0
        private class MyHandler(val activity: UnboxParityActivity?): Handler() {
            override fun handleMessage(msg: Message?) {
                when (msg?.what) {
                    HIDE_TOOLBAR -> activity?.hideToolBr()
                    else -> {}
                }
            }
        }
    ...
    private fun delayHideSystemUI() {
        handler.removeMessages(HIDE_TOOLBAR)
        handler.sendEmptyMessageDelayed(HIDE_TOOLBAR, 3000)
    }
